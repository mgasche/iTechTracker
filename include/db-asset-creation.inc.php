...
if ($_SERVER['REQUEST_METHOD'] == "POST") {
    ...
    // Trimmen und htmlspecialchars
    $purchase_date = isset($_POST['purchase_date']) ? htmlspecialchars(trim($_POST['purchase_date'])) : '';
    ...
    $warranty_end = isset($_POST['warranty_end']) ? htmlspecialchars(trim($_POST['warranty_end'])) : '';
    $image_path = isset($_POST['image_path']) ? htmlspecialchars(trim($_POST['$image_path'])) : '';


    // Wenn keine Fehler Eintrag in DB erstellen.
    if (empty($error)) {
        // Query erstellen
        ...
        // Parameter binden
        $stmt->bind_param("isssssssssss", $user_id, $device_name, $model, $manufacturer, $purchase_date, $price, $color, $processor, $ram, $storage, $warranty_end, $image_path);
